    ...
    <div class="container" style="margin-top: 20px;">
            ...
            <table class="table table-striped" id="myTable">
                ...
                <tbody>
                    @foreach ($events as $event)
                        <tr>
                            <td><a href="@auth{{ route('lecturer.join.show', $event) }}@endauth">{{ $event->event }}</td>
                            @if ($event->type == 0)
                                <td>Lecturer Exchange</td>
                            @else
                                <td>Lecturer Excursion</td>
                            @endif
                            <td>{{ $event->event_date }}</td>
                            @if ($event->status == 4)
                                <td>
                                    <form action="{{ route('lecturer.join.group') }}" method="POST">
                                        @csrf
        ...
        @elseif($pages == 'showit')
            <div
                ...
                $type = 'lecturer Exchange';
                } else {
                $type = 'lecturer Excursion';
                } ?>
                <div class="form-event" style="margin-bottom: 10px;">
                    <label>Type :</label>
